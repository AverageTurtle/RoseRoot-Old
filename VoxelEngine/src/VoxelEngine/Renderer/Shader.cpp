...
namespace VoxelEngine {
	...
	{
		VE_CORE_ASSERT(!Exist(name), "Shader already exist!");
	...
	Ref<VoxelEngine::Shader> ShaderLibrary::Load(const std::string& filepath)
	{
	...
	Ref<VoxelEngine::Shader> ShaderLibrary::Load(const std::string& name, const std::string& filepath)
	{
		auto shader = Shader::Create(filepath);
		Add(name, shader);
		return shader;
	...
	Ref<VoxelEngine::Shader> ShaderLibrary::Get(const std::string& name)
	{
		VE_CORE_ASSERT(Exist(name), "Shader not found");
