...
namespace RoseRoot {
	...
	std::string OpenGLShader::ReadFile(const std::string& filepath)
	{
		...
		{
			RR_CORE_WARN("Could not open file '{0}'", filepath);
		}
		return result;
	}
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			RR_CORE_ASSERT(ShaderTypeFromString(type), "Invalid shader type specified");
	...
	void OpenGLShader::Compile(const std::unordered_map<GLenum, std::string>& shaderSources)
	{
		...
		for (auto& kv : shaderSources)
		{
			...
			if (isCompiled == GL_FALSE)
			{
				...
				RR_CORE_ASSERT(false, "Shader compilation failure!");
				break;
			}
		...
		if (isLinked == GL_FALSE)
		{
			...
			RR_CORE_ERROR("{0}", infoLog.data());
			RR_CORE_ASSERT(false, "Shader link failure!");
			return;
		}
